...
function header(){const initial=(state.name.trim()[0]||"旅").toUpperCase();return `<header class="topbar"><button class="brand brand-button" data-action="home"><span class="brand-mark">S</span>Spanish OS</button><nav class="nav-links"><button class="active" data-action="home">首頁</button><button data-action="map">場景地圖</button></nav><div class="header-actions"><div class="xp-counter"><span>XP</span><strong>${state.xp}</strong></div><button class="settings-trigger" data-action="settings" aria-label="設定">⚙</button><span class="avatar" title="學習者：${escapeHtml(state.name||"旅人")}">${escapeHtml(initial)}</span></div></header>`;}
...
function quiz(){const l=lessons[state.lesson],q=l.questions[state.step],progress=((state.step+(state.selected!==null?1:0))/l.questions.length)*100,receptive=q.answer!==q.phrase;return `${header()}<section class="lesson-screen"><div class="lesson-top"><button data-action="home" aria-label="離開課程">×</button><div class="lesson-track"><i style="width:${progress}%"></i></div><span class="combo">🔥 ${state.combo} COMBO</span></div><article class="quiz-card"><small>第 ${state.lesson+1} 課 · ${state.step+1}/${l.questions.length}</small>${receptive?`<div class="sound-row"><button data-speak="${q.phrase}" aria-label="播放發音">▶</button><button data-speak-slow="${q.phrase}">慢速播放</button></div><h2>${q.phrase}</h2>`:'<span class="eyebrow question-type">情境選句</span>'}<p>${q.prompt}</p><div class="answer-grid">${q.options.map((o,i)=>{let cls="";if(state.selected!==null)cls=o===q.answer?"correct":i===state.selected?"wrong":"muted";return `<button class="${cls}" data-answer="${i}" ${state.selected!==null?"disabled":""}><b>${String.fromCharCode(65+i)}</b>${o}</button>`}).join("")}</div>${feedback(q)}</article></section>${settings()}`;}
